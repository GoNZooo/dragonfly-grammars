...
config.lang.win_selector = Item(
    "window | win | [window] <win_names>",
    doc="Partial command for specifying a window; must contain the <win_names> extra.")
config.lang.mon_selector = Item(
    "(this | current) monitor | [monitor] <mon_names>",
...
config.settings.grid = Item(
    4, doc="The number of grid divisions a monitor is divided up into when placing windows.")
...
winControlGrammar.add_rule(NameWinRule())
...
winControlGrammar.add_rule(FocusWinRule())
...
winControlGrammar.add_rule(FocusTitleRule())
...
winControlGrammar.add_rule(TranslateRule())
...
winControlGrammar.add_rule(ResizeRule())
...
winControlGrammar.add_rule(StretchRule())


winControlGrammar.load()


def unload():
    global winControlGrammar
    if winControlGrammar:
        winControlGrammar.unload()
    winControlGrammar = None
